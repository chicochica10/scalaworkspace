...
object l5_pouring_water_optimizacion1 {
  ...
  class Pouring(capacities: Capacities) {
    ...
    // el problema es que al construir los nuevos paths estamos incluyendo paths que ya se han visitado y que llevan a states ya generados
    // y vamos a excluir esos paths
    def from(paths: Set[Path], exploredStates: Set[States]): Stream[Set[Path]] = {
      if (paths.isEmpty) Stream.Empty
        ...
        //<- la funcion map toma todos los nuevos paths, les calcula su estado final
        //... ese estado final es concacatedado al conjunto de estados finales anterior
  ...
  val capacities = Vector(4, 9)                   //> capacities  : scala.collection.immutable.Vector[Int] = Vector(4, 9)
    val pouring = new Pouring(capacities)         //> pouring  : week7.l5_pouring_water_optimizacion1.Pouring = week7.l5_pouring_
                                                  //| water_optimizacion1$Pouring@1795f1cc
  // sin optimizar tarda
  pouring.pathsSet.take(3).toList                 //> res0: List[Set[week7.l5_pouring_water_optimizacion1.pouring.Path]] = List(S
  ...
  pouring.solution(6)                             //> res1: Stream[week7.l5_pouring_water_optimizacion1.pouring.Path] = Stream(fi
